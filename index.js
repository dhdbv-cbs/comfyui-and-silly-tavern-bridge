// index.js - V50.0 (Sequential Parsing & DOM Tree Walker Injection)

const extensionName = "comfyui-flow-bridge-v50-0";
let comfyURL = localStorage.getItem("cf_v50_api_url") || ""; 
let clientId = "client_" + Date.now(); 

// --- LLM 配置 ---
let llmSettings = {
    url: localStorage.getItem("cf_v50_llm_url") || "https://api.openai.com/v1",
    key: localStorage.getItem("cf_v50_llm_key") || "",
    model: localStorage.getItem("cf_v50_llm_model") || "gpt-3.5-turbo",
    systemPrompt: localStorage.getItem("cf_v50_llm_sys") || "你是一位 Stable Diffusion 提示词专家..."
};
...
function initPresetsData() {
    try {
        presets.jailbreak = JSON.parse(localStorage.getItem("cf_v50_jailbreak") || "[]");
        presets.task = JSON.parse(localStorage.getItem("cf_v50_task") || "[]");
        presets.char = JSON.parse(localStorage.getItem("cf_v50_char") || "[]");
    } catch (e) { presets = { jailbreak: [], task: [], char: [] }; }
}
initPresetsData();

// --- 动态提示词面板 ---
let promptPanels = JSON.parse(localStorage.getItem("cf_v50_prompt_panels") || "[]");
...
let nodeDefinitions = {};    
let socketCache = {};        
let currentApiName = localStorage.getItem("cf_v50_api_name") || "未加载";
let currentVisName = localStorage.getItem("cf_v50_vis_name") || "未加载";
let capturedContext = ""; 
let socket = null; 

let apiPresets = JSON.parse(localStorage.getItem("cf_v50_api_presets") || "[]");
let visPresets = JSON.parse(localStorage.getItem("cf_v50_vis_presets") || "[]");
...
function cleanupOldVersion() {
    ...
    const oldStyle = document.getElementById("comfyui-v50-style"); if(oldStyle) oldStyle.remove();
    const oldToast = document.getElementById("cf-toast"); if(oldToast) oldToast.remove();
    document.querySelectorAll('.cf-inject-wrapper').forEach(el => el.remove());
}
cleanupOldVersion();

function injectStyles() {
    const styleId = "comfyui-v50-style";
    ...
    style.textContent = `
        ...
        .cf-inject-btn.loading { background: #e67700; cursor: wait; }
        .cf-result-img { 
            max-width: 300px; max-height: 300px; border-radius: 8px; 
            border: 2px solid #339af0; box-shadow: 0 4px 15px rgba(0,0,0,0.5); 
            display: block; margin-top: 5px; animation: cf-fade-in 0.5s; cursor: zoom-in;
        }
...
function initSystem() {
    ...
    const savedApi = localStorage.getItem("cf_v50_api_json");
    const savedVis = localStorage.getItem("cf_v50_visual_json");
...
function captureLastTwoMessages() {
    ...
    const contextText = lastTwo.map(el => el.innerText.trim()).join("\n\n---\n\n");
...
async function handleExecutionSuccess(promptId) {
    ...
    try {
        ...
        if (imgFilename) {
            ...
            wrapper.innerHTML = `<img src="${src}" class="cf-result-img" onclick="window.open(this.src, '_blank')">`;
            showToast("✅ 图片生成完毕并已插入");
...
window.exportBackup = function() {
    const backupData = {
        ...
        currentApiJson: localStorage.getItem("cf_v50_api_json") || null,
        currentVisJson: localStorage.getItem("cf_v50_visual_json") || null,
...
window.handleBackupImport = function(input) {
    ...
    reader.onload = (e) => {
        try {
            ...
            localStorage.setItem("cf_v50_jailbreak", JSON.stringify(presets.jailbreak));
            localStorage.setItem("cf_v50_task", JSON.stringify(presets.task));
            localStorage.setItem("cf_v50_char", JSON.stringify(presets.char));
            localStorage.setItem("cf_v50_prompt_panels", JSON.stringify(promptPanels));
            localStorage.setItem("cf_v50_api_presets", JSON.stringify(apiPresets));
            localStorage.setItem("cf_v50_vis_presets", JSON.stringify(visPresets));
            localStorage.setItem("cf_v50_api_name", currentApiName);
            localStorage.setItem("cf_v50_vis_name", currentVisName);
        ...
        <div id="cf-panel">
            <div class="cf-head">
                <div style="display:flex;align-items:center;">
                    ...
                    <span>Comfy Bridge V50.0 (DOM Walker & Seq Parser)</span>
                </div>
                ...
                <div id="view-setup" class="cf-view cf-scroll-y">
                     ...
                     <div class="cf-group">
                        <div style="display:flex; gap:15px; align-items:center;">
                            ...
                            <input id="cf-url" class="cf-inp" value="${comfyURL}" placeholder="http://127.0.0.1:8188" onchange="localStorage.setItem('cf_v50_api_url', this.value)">
...
function openPanel() {
    document.getElementById("cf-mask").classList.add("show");
    if(capturedContext && document.getElementById("cf-test-user-input")) document.getElementById("cf-test-user-input").value = capturedContext;
    if(visualWorkflow) { switchTab('visual', document.querySelectorAll(".cf-tab")[0]); setTimeout(drawCanvas, 100); } 
    else { switchTab('setup', document.querySelectorAll(".cf-tab")[1]); }
}
...
// ----------------------------------------
// 5. 核心注入逻辑 (V50: 顺序解析 + DOM Tree Walker)
// ----------------------------------------

function clearInjectedButtons() {
    document.querySelectorAll('.cf-inject-wrapper').forEach(btn => btn.remove());
}

// V50: DOM Walker - 精确插入文本节点后
function injectNodeAfterText(rootElement, searchText, nodeToInject) {
    ...
    if (endIdx >= textMap.length) {
        rootElement.appendChild(nodeToInject); // 文本末尾
    } else {
        const mapEntry = textMap[endIdx - 1]; // 目标文本的最后一个字符
...
function injectButtonIntoChat(locatorText, promptText) {
    if (!locatorText || !promptText) return;
    const messages = document.querySelectorAll('.mes_text');
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
        ...
        if (messageEl.innerText.includes(cleanLocator)) {
            ...
            if (injectNodeAfterText(messageEl, cleanLocator, wrapper)) {
                console.log("ComfyBridge: Injected precisely after:", cleanLocator);
                break;
            }
        }
    }
...
window.handleInjectedClick = function(e, btnId, promptText) {
    ...
    if(posPanel) {
        ...
        showToast(`✅ Prompt 已填入`);
    }
    const btn = document.getElementById(btnId);
    if(btn) {
        btn.innerHTML = `<span class="fa-solid fa-spinner fa-spin"></span> 提交中...`;
...
function parseAndRenderOutput(text) {
    ...
    const dataRegex = /<资料>([\s\S]*?)<\/资料>/g; // 资料单独处理
    ...
    while ((match = regex.exec(text)) !== null) {
        const tag = match[1]; // <角色> or <定位>
        const content = match[2].trim();
        
        if (tag === '<角色>') {
            currentRole = content; // 暂存角色 Prompt，等待定位
        } else if (tag === '<定位>') {
            // 如果有角色在前，则配对成功
            if (currentRole) {
                sceneIndex++;
                ...
                // 执行注入
                injectButtonIntoChat(locationText, promptText);
...
window.handleDataAction = function(cardId) {
    ...
    if(existingIdx >= 0) { presets.char[existingIdx].content = content; presets.char[existingIdx].active = true; showToast(`🔄 已更新角色: ${extractedName}`); } 
    else { presets.char.push({ id: Date.now(), name: extractedName, content, active: true }); showToast(`💾 已新建角色: ${extractedName}`); }
...
function savePanels() { localStorage.setItem("cf_v50_prompt_panels", JSON.stringify(promptPanels)); }
...
window.addPreset = function(type) { const nameEl = document.getElementById(`inp-${type}-name`); const contentEl = document.getElementById(`inp-${type}-content`); const name = nameEl ? nameEl.value.trim() : ""; const content = contentEl ? contentEl.value.trim() : ""; if(!name || !content) return alert("请填写名称和内容"); const existingIdx = presets[type].findIndex(p => p.name === name); if(existingIdx >= 0) { if(!confirm(`预设 [${name}] 已存在，是否更新内容？`)) return; presets[type][existingIdx].content = content; if(type !== 'char') presets[type].forEach(p => p.active = false); presets[type][existingIdx].active = true; showToast("🔄 预设已更新"); } else { if(type !== 'char') presets[type].forEach(p => p.active = false); presets[type].push({ id: Date.now(), name, content, active: true }); showToast("💾 预设已新建"); } savePresetsToStorage(type); renderPresetList(type); updateSystemPromptPreview(); }
window.loadPresetToEditor = function(type, id) { const item = presets[type].find(p => p.id === id); if(item) { document.getElementById(`inp-${type}-name`).value = item.name; document.getElementById(`inp-${type}-content`).value = item.content; window.togglePreset(type, id); } }
function renderPresetList(type) { const container = document.getElementById(`list-${type}`); if(!container) return; container.innerHTML = ""; presets[type].forEach(item => { const activeClass = item.active ? "active" : ""; container.innerHTML += `<div class="cf-preset-item ${activeClass}" onclick="window.loadPresetToEditor('${type}', ${item.id})"><div style="flex:1; overflow:hidden;"><div style="font-weight:bold; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${item.name}</div><div style="font-size:10px; color:#666; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${item.content.substring(0,40).replace(/\n/g, ' ')}...</div></div><button class="cf-btn red" style="padding:2px 6px; margin-left:5px; font-size:10px;" onclick="event.stopPropagation(); window.deletePresetItem('${type}', ${item.id})">×</button></div>`; }); }
...
window.togglePreset = function(type, id) { if(type === 'char') { const p = presets[type].find(i => i.id === id); if(p) p.active = !p.active; } else { presets[type].forEach(p => p.active = (p.id === id)); } savePresetsToStorage(type); renderPresetList(type); updateSystemPromptPreview(); }
window.deletePresetItem = function(type, id) { if(!confirm("确认删除?")) return; presets[type] = presets[type].filter(p => p.id !== id); savePresetsToStorage(type); renderPresetList(type); updateSystemPromptPreview(); }
function savePresetsToStorage(type) { try { localStorage.setItem(`cf_v50_${type}`, JSON.stringify(presets[type])); } catch(e) { console.error(e); } }
...
async function checkConnection(url) {
    ...
    try { 
        ...
        if(r.ok) { nodeDefinitions = await r.json(); comfyURL = u; localStorage.setItem("cf_v50_api_url", u); b.innerText = "✅"; b.className = "cf-btn green"; if(visualWorkflow) drawCanvas(); fetchResources(); initWebSocket(); } else throw new Error(); 
...
async function runWorkflow(triggerBtnId = null) {
    ...
    try { 
        ...
        if(res.ok) { 
            ...
            if (triggerBtnId) {
                ...
                if (btn) {
                    ...
                    btn.innerHTML = `<span class="fa-solid fa-spinner fa-spin"></span> 生成中 (0%)...`;
                }
            }
        } 
    } catch(e) { 
        ...
        if(triggerBtnId) {
            const btn = document.getElementById(triggerBtnId);
            if(btn) { btn.innerHTML = "❌ 失败"; btn.disabled = false; btn.classList.remove("loading"); }
        }
    } finally { 
...
// ... Common & Canvas ...
window.updateLlmSetting = function(key, val) { llmSettings[key] = val; localStorage.setItem(`cf_v50_llm_${key}`, val); }
window.saveLlmSettingsManual = function() { localStorage.setItem("cf_v50_llm_url", llmSettings.url); localStorage.setItem("cf_v50_llm_key", llmSettings.key); localStorage.setItem("cf_v50_llm_model", llmSettings.model); localStorage.setItem("cf_v50_llm_sys", llmSettings.systemPrompt); showToast("💾 设置已保存"); }
...
function renderNodeWidgets(nid, type, inps, cont) { if(!nodeDefinitions[type]) return; const def = nodeDefinitions[type]; const idefs = { ...(def.input?.required || {}), ...(def.input?.optional || {}) }; for (const k in inps) { let v = inps[k]; if (Array.isArray(v)) continue; let idef = idefs[k]; if(!idef) continue; let t = idef[0]; let h = ""; if (k === 'image' && typeof v === 'string') { const src = v.startsWith("http") ? v : `${comfyURL}/view?filename=${v}&type=input`; h = `<div class="cf-preview-box"><img src="${src}" id="pv-${nid}-${k}" class="cf-preview-img"></div><div class="cf-widget-row"><span class="cf-widget-label">Image</span><label class="cf-mini-btn">⬆<input type="file" accept="image/*" style="display:none" onchange="uploadHandler(this, '${nid}', '${k}')"></label></div>`; } else if (Array.isArray(t)) { h = `<div class="cf-widget-row"><span class="cf-widget-label">${k}</span><select class="cf-sel cf-sync" data-id="${nid}" data-key="${k}">`; t.forEach(o => h += `<option value="${o}" ${o===v?"selected":""}>${o}</option>`); h += `</select></div>`; } else if (k.includes("seed") || k === "seed") { h = `<div class="cf-widget-row"><span class="cf-widget-label">Seed</span><input type="number" class="cf-inp cf-sync" data-id="${nid}" data-key="${k}" value="${v}" style="width:70px;"><button class="cf-mini-btn" onclick="randomSeed('${nid}', '${k}')">🎲</button></div>`; } else if (t === "INT" || t === "FLOAT" || typeof v === 'number') { h = `<div class="cf-widget-row"><span class="cf-widget-label">${k}</span><input type="number" step="any" class="cf-inp cf-sync" data-id="${nid}" data-key="${k}" value="${v}"></div>`; } else if (t === "STRING" || typeof v === 'string') { if (v.length > 20 || k.includes("text")) { h = `<div style="padding:2px 6px;"><span class="cf-widget-label">${k}</span><textarea class="cf-inp cf-sync" data-id="${nid}" data-key="${k}" rows="2">${v}</textarea></div>`; } else { h = `<div class="cf-widget-row"><span class="cf-widget-label">${k}</span><input type="text" class="cf-inp cf-sync" data-id="${nid}" data-key="${k}" value="${v}"></div>`; } } if(h) cont.insertAdjacentHTML('beforeend', h); } cont.querySelectorAll(".cf-sync").forEach(e => { e.addEventListener('input', (ev) => { let v = ev.target.value; if(ev.target.tagName === 'INPUT' && ev.target.type === 'number') v = Number(v); if(apiWorkflow && apiWorkflow[nid]) { apiWorkflow[nid].inputs[ev.target.dataset.key] = v; localStorage.setItem("cf_v50_api_json", JSON.stringify(apiWorkflow)); } }); }); }
function initCanvasControls() { const vp = document.getElementById("cf-canvas-viewport"); const w = document.getElementById("cf-canvas-world"); const up = () => { w.style.transform = `translate(${canvasState.x}px, ${canvasState.y}px) scale(${canvasState.scale})`; }; vp.addEventListener("mousedown", (e) => { const h = e.target.closest(".cf-canvas-node-head"); if(h) { canvasState.isDraggingNode = true; canvasState.draggedNodeId = parseInt(h.dataset.nodeId); canvasState.lastX = e.clientX; canvasState.lastY = e.clientY; h.parentElement.style.zIndex = ++canvasState.zIndexCounter; return; } if(!['INPUT','SELECT','TEXTAREA','BUTTON'].includes(e.target.tagName)) { canvasState.isPanning = true; canvasState.lastX = e.clientX; canvasState.lastY = e.clientY; vp.classList.add("panning"); } }); window.addEventListener("mousemove", (e) => { if(canvasState.isDraggingNode && canvasState.draggedNodeId !== null) { const dx = (e.clientX - canvasState.lastX) / canvasState.scale; const dy = (e.clientY - canvasState.lastY) / canvasState.scale; const n = visualWorkflow.nodes.find(n => n.id === canvasState.draggedNodeId); if(n) { n.pos[0] += dx; n.pos[1] += dy; const el = document.getElementById(`node-${n.id}`); if(el) { el.style.left = n.pos[0] + "px"; el.style.top = n.pos[1] + "px"; } updateSocketCache(); drawLinks(); } canvasState.lastX = e.clientX; canvasState.lastY = e.clientY; } else if(canvasState.isPanning) { canvasState.x += e.clientX - canvasState.lastX; canvasState.y += e.clientY - canvasState.lastY; canvasState.lastX = e.clientX; canvasState.lastY = e.clientY; up(); } }); window.addEventListener("mouseup", () => { canvasState.isPanning = false; canvasState.isDraggingNode = false; vp.classList.remove("panning"); if(visualWorkflow) localStorage.setItem("cf_v50_visual_json", JSON.stringify(visualWorkflow)); }); vp.addEventListener("wheel", (e) => { e.preventDefault(); const d = e.deltaY > 0 ? 0.9 : 1.1; canvasState.scale = Math.min(Math.max(0.1, canvasState.scale * d), 5); up(); }); }
...
window.deletePreset = function(t, i) { if(!confirm("删除?")) return; const l = (t === 'api') ? apiPresets : visPresets; l.splice(i, 1); localStorage.setItem(`cf_v50_${t}_presets`, JSON.stringify(l)); renderPresets(t); }
...
function parseApiJson(s, sv=true) { try { apiWorkflow = JSON.parse(s); document.getElementById("cf-json-api-raw").value = s; if(sv) localStorage.setItem("cf_v50_api_json", s); renderPromptPanels(); } catch(e){} }
function parseVisualJson(s, sv=true) { try { visualWorkflow = JSON.parse(s); document.getElementById("cf-json-vis-raw").value = s; if(sv) localStorage.setItem("cf_v50_visual_json", s); drawCanvas(); } catch(e){} }
function clearApiData() { apiWorkflow=null; localStorage.removeItem("cf_v50_api_json"); currentApiName=""; document.getElementById("cf-status-api").innerText=""; document.getElementById("cf-json-api-raw").value=""; renderPromptPanels(); }
function clearVisData() { visualWorkflow=null; localStorage.removeItem("cf_v50_visual_json"); currentVisName=""; document.getElementById("cf-status-vis").innerText=""; document.getElementById("cf-canvas-nodes").innerHTML=""; document.getElementById("cf-canvas-links").innerHTML=""; document.getElementById("cf-json-vis-raw").value=""; }
window.handleFileImport = function(e, t) { const f = e.target.files[0]; if(!f) return; const r = new FileReader(); r.onload = (ev) => { if(t === 'api') { currentApiName = f.name; localStorage.setItem("cf_v50_api_name", currentApiName); parseApiJson(ev.target.result); document.getElementById("cf-status-api").innerText = currentApiName; } else { currentVisName = f.name; localStorage.setItem("cf_v50_vis_name", currentVisName); parseVisualJson(ev.target.result); document.getElementById("cf-status-vis").innerText = currentVisName; } }; r.readAsText(f); }
